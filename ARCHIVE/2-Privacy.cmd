...
echo.
echo --- Clear Windows Product Key
...
%SystemRoot%\System32\Unlocker\Unlocker.exe "%Windir%\System32\Narrator.exe" /R "%Windir%\System32\Narrator.bak" /S
...
PowerShell -Command "$key = 'HKLM:SOFTWARE\Microsoft\Windows\CurrentVersion\Diagnostics\DiagTrack\SettingsRequests'; Get-ChildItem $key | foreach { Set-ItemProperty -Path \"$key\$($_.pschildname)\" -Name ETag -Value '' -ErrorAction SilentlyContinue -Verbose }"
PowerShell -Command "$key = 'HKLM:SOFTWARE\Microsoft\Windows\CurrentVersion\Diagnostics\DiagTrack\SettingsRequests'; Get-ChildItem $key | foreach { Set-ItemProperty -Path \"$key\$($_.pschildname)\" -Name ETagQueryParameters -Value '' -ErrorAction SilentlyContinue -Verbose }"
...
netsh advfirewall firewall add rule name="System" program="%systemroot%\System32\svchost.exe" dir=out enable=yes action=Allow profile=any
echo.
