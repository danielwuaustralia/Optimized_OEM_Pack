...
set DiskID=
set "_PS_CMD_=Get-Disk ^| where ^{$_.BusType -EQ 'USB' -and $_.Number -EQ !_T!^} ^| select Number"
...
IF Not defined _L (
	...
	(2>nul fsutil fsinfo drivetype !_T! | >nul find /i "Removable Drive") && goto :ID_BLAT_MAN
	(2>nul powershell -nop -c "get-partition | where DriveLetter -eq '!_T:~0,1!' | get-disk | select BusType | Format-Wide" | >nul find /i "USB" ) && goto :ID_BLAT_MAN
	
	echo.
	echo ERROR ##### Target Is Not A Removeable Drive
	echo.
	goto :WTF
)
...
IF defined _L (
	...
	if not defined _T (
		...
		(2>nul fsutil fsinfo drivetype !_S! | >nul find /i "Removable Drive") && goto :ID_BLAT_MAN
		(2>nul powershell -nop -c "get-partition | where DriveLetter -eq '!_S:~0,1!' | get-disk | select BusType | Format-Wide" | >nul find /i "USB" ) && goto :ID_BLAT_MAN
		
		echo.
		echo ERROR ##### Source Is Not A Removeable Drive
		echo.
		goto :WTF
	)
...
set "_PS_CMD_=Get-Disk | where {$_.BusType -EQ 'USB'} | select Number,Size,FriendlyName | Foreach {$sz = $_.size/1GB; Write-Host 'Disk -'$_.Number', Total -'$sz 'GB , Name -' $_.FriendlyName}"
